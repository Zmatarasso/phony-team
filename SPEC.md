...
### 9.4 Multi-Repo Targeting (Implementation Extension)

When configured, Symphony can target multiple git repositories within a single folder instead of a
single `repo_url`. This enables automatic routing of issues to the correct repo(s) before agent
dispatch.

#### 9.4.1 Configuration

A new optional field `workspace.repo_root` supplements the existing single-repo population approach:

```yaml
workspace:
  root: ~/symphony_workspaces
  repo_root: ~/code            # directory containing multiple git repos
```

- If `repo_root` is set, it takes precedence over any single-repo population mechanism.
- `repo_root` points to a local directory containing multiple git repositories.
- Symphony scans **only top-level subdirectories** of `repo_root` for directories containing a
  `.git/` folder. It does not recurse deeper.
- The scan builds a **repo registry** at startup: for each discovered repo, Symphony records the
  directory name, path, remote URL, default branch, and a summary derived from the repo's README (if
  present) and primary language(s).
- All repos are assumed to be accessible with the same git credentials.

#### 9.4.2 Repo Resolution

A **Repo Resolver** component runs before agent dispatch for each issue. Its job is to determine
which repo(s) an issue targets.

Input sources (checked in priority order):

1. **Tracker custom field** — If a custom field (e.g., `target_repos`) exists on the issue and is
   populated, its value is used directly. This is the highest-priority signal and skips further
   resolution entirely.
2. **Issue description and comments** — The resolver scans the issue description and all comments
   for explicit repo references (repo names, GitHub URLs, directory names matching entries in the
   repo registry).
3. **LLM-based matching** — If no explicit repo references are found in the above sources, the
   resolver makes a lightweight LLM call (~1 turn) to match the issue (title, description, labels,
   components) against the repo registry. The prompt includes repo names, descriptions/README
   summaries, and primary languages.

Output states:

- **Single repo (confident)** — Exactly one repo is identified. Proceed directly to agent dispatch.
- **Multiple repos (confident)** — The issue clearly requires changes across N repos (e.g., "update
  the API contract between service-a and service-b"). The resolver attaches the full repo list to
  the issue context and proceeds to dispatch.
- **Ambiguous** — The resolver is not confident about which repo(s) the issue targets. The issue is
  transitioned to **Blocked** state with a tracker comment listing the candidate repos and asking for
  human clarification. No agent work begins.

#### 9.4.3 Ambiguous Resolution Recovery

On subsequent poll cycles, if a human has:

- Populated the `target_repos` custom field, OR
- Added a comment specifying the target repo(s)

...the resolver re-evaluates and may unblock the issue, allowing dispatch to proceed.

#### 9.4.4 Multi-Repo Workspace Layout

For single-repo issues, the workspace contains one clone of the target repo (same as the standard
single-repo behavior).

For multi-repo issues, the workspace directory contains multiple subdirectories, one per target repo:

```
<workspace.root>/<sanitized_issue_identifier>/
  ├── service-a/     # clone of service-a
  └── service-b/     # clone of service-b
```

The `after_create` and `before_run` hooks iterate over all target repos in the workspace.

#### 9.4.5 Orchestrator Integration

A repo resolution gate is added to dispatch eligibility (Section 8.2):

- Before dispatching an issue, check repo resolution (if not already resolved).
- If resolved: attach repo list to issue context, proceed to dispatch.
- If ambiguous: transition to Blocked, post tracker comment, skip dispatch.
- Resolution results are cached in-memory per issue (`repo_resolution` map on orchestrator state).
  This cache is not persisted; it is re-computed on restart.

#### 9.4.6 Agent Prompt and PR Behavior

For multi-repo issues, the prompt template should include information about all target repos so the
agent knows which subdirectories are available and what each repo contains.

The agent creates separate branches and PRs per repo. The tracker comment includes links to all PRs.
The issue transitions to the handoff state only after all PRs are created.
